...
class DataFrameSelector(sklearn.base.BaseEstimator, sklearn.base.TransformerMixin):
    ...
    def __init__(self, do_predictors=True, do_numerical=True):
        # heart fields
        #age, sex, cp, trtbps, chol, fbs, restecg, thalaachh, exng, oldpeak, slp, caa, thall
        self.mCategoricalPredictors = []
        self.mNumericalPredictors = ["age", "sex", "cp", "trtbps", "chol", "fbs", "restecg", "thalachh", "exng", "oldpeak", "slp", "caa", "thall"]
        self.mLabels = ["output"]
...
def make_decision_tree_fit_pipeline(my_args):
    ...
    items.append(("model", sklearn.tree.DecisionTreeClassifier(criterion="entropy", max_depth=2)))
